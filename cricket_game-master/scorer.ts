...
//batsman calculation
   ...
   let R = batsman.getRuns();
   let B = batsman.getBalls();
 
    
    console.log(chalk.blue(batsman.name) + "       " + (chalk.red(ball.dismissalType)) + "     " + (chalk.green(batsman.numberOfRunsScored)) + "     (" + (chalk.blue(batsman.numberOfBallsFaced) + ")")  + "   (SR - " + chalk.red(((R/B)*100).toFixed(2)) + " )");
...
// printing batsman table 
  ...
  for(var i=0;i<keys.length;i++)
  {
      ...
      let R = batsman.getRuns();
      let B = batsman.getBalls();
    console.log(chalk.yellow(batsman.name) +chalk.yellow("*") + (chalk.green("    Not Out     ") + (chalk.yellow(batsman.numberOfRunsScored)) + "    (" + (chalk.blue(batsman.numberOfBallsFaced)) + ")") + "   (SR - " + chalk.red(((R/B)*100).toFixed(2)) + " )");
  }
  console.log("\n");
